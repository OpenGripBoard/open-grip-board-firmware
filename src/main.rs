...
use esp_idf_svc::mqtt::{
    self,
    client::{EspMqttClient, EspMqttConnection},
};
use hx711::Hx711;
...
fn main() -> Result<()> {
    ...
    // Touchscreen
    let i2c = init_i2c(pins.gpio18, pins.gpio8, i2c0)?;

    // Load cell
    let mut hx711 = init_load_cell(pins.gpio22, pins.gpio23)?;
    ...
    // Setup hx711, give it some time before first read, Poll in background thread
    ...
    loop {
        ...
        // connect mqtt
        if model.wifi_is_connected && mqtt_client.is_none() {
            let (client, _) = init_mqtt()?;
        ...
        if model.is_recording {
            ...
            if let Some(reading) = latest {
                ...
                if let Some(client) = &mut mqtt_client {
                    let payload = (reading * 1000_f32) as i32;
                    let topic = &format!("{}/sensor/measurement", BOARD_NAME);
                    client.publish(
                        topic,
                        mqtt::client::QoS::AtMostOnce,
                        false,
